...
def display_world(world_size, position, landmarks=None, true_landmarks=None):
    ...
    if(true_landmarks is not None):
        # loop through all path indices and draw a dot (unless it's at the car's location)
        for pos in true_landmarks:
            if(pos != position):
                ax.text(pos[0], pos[1], 'o', ha='center', va='center', color='yellow', fontsize=20)
...
def make_data(N, num_landmarks, world_size, measurement_range, motion_noise, 
    ...
    while not complete:
        ...
        print(r)  # RK: print start position

        data = []

        seen = [False for row in range(num_landmarks)]
        
        # RK: I think initialization of data and seen must be outside while loop. If not, these will be reinitialized
        # at the second iteration, while the start position on the second iteraton is unknown! So either robot needs to 
        # be placed as start position again, or we should accumulate the data of the second iteration to the data of the
        # first iteration.
